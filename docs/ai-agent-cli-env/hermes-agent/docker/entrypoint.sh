...
# Docker starts the entrypoint as root, so HOME may still be /root after gosu.
# Keep CLI tools such as Cursor Agent writing config into the mounted profile.
...
if [ "$(id -u)" = "0" ]; then
    ...
    # If the host Docker socket is mounted, grant the runtime user access to
    # the socket group so the bundled docker-cli can drive the host daemon.
    if [ -S /var/run/docker.sock ]; then
        ...
        if [ -n "$docker_sock_gid" ]; then
            ...
            if [ -n "$docker_sock_group" ]; then
                usermod -aG "$docker_sock_group" hermes 2>/dev/null || true
            fi
        fi
    fi
